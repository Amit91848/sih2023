...
def create_file(db: Session, name: str, url: str, key: str, user_id: int, isLocal: bool, size: int, isPdf: bool = True):
    db_file = File(name=name, url=url, key=key, user_id=user_id, isLocal=isLocal, size=size, isPdf=isPdf)
    db.add(db_file)
    db.commit()
    db.refresh(db_file)
    return db_file
